...
class BaseDataset(metaclass=ABCMeta):
    ...
    def get_tf_datasets(self):
        """최적화된 TF 데이터셋 반환"""
        if not hasattr(self, '_optimized_datasets'):
            self._optimized_datasets = {}
            for split_name, dataset in self.tf_splits.items():
                # 캐싱 적용
                if self.config.get('cache_in_memory', False):
                    dataset = dataset.cache()
                    
                # 배치 처리 최적화
                batch_size = self.config.get(f'{split_name}_batch_size', 
                                            self.config.get('batch_size', 32))
                dataset = dataset.batch(batch_size)
                
                # 프리페치로 데이터 로딩 최적화
                prefetch_size = self.config.get('prefetch_size', tf.data.experimental.AUTOTUNE)
                dataset = dataset.prefetch(prefetch_size)
                
                self._optimized_datasets[split_name] = dataset
                
        return self._optimized_datasets
