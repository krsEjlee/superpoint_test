...
def train(config, n_iter, output_dir, pretrained_dir=None, checkpoint_name='model.ckpt'):
    ...
    # session_config 전달하지 않고 함수 내에서 처리하도록 함
    with _init_graph(config) as net:
        if pretrained_dir is not None:
            # pretrained_dir가 디렉토리인지 파일인지 확인하고, 
            # 둘 중 하나라도 지원하도록 합니다.
        ...
        # 메모리 효율성을 위한 배치 크기 조정
        if config['model'].get('batch_size', 1) > 32:
            tf.logging.warning('Large batch size detected. Consider reducing to improve memory usage.')
        
        # 제한된 메모리에서 그래디언트 축적 시뮬레이션
        grad_accumulation_steps = config.get('grad_accumulation_steps', 1)
        if grad_accumulation_steps > 1:
            tf.logging.info(f"Using gradient accumulation with {grad_accumulation_steps} steps")
        ...
        try:
            ...
            tf.logging.info('Got Keyboard Interrupt, saving model and closing.')
...
@contextmanager
def _init_graph(config, with_dataset=False, session_config=None):
    set_seed(config.get('seed', int.from_bytes(os.urandom(4), byteorder='big')))
    n_gpus = get_num_gpus()
    logging.info('Number of GPUs detected: {}'.format(n_gpus))

    # 세션 설정이 제공되지 않은 경우 기본 설정 생성
    if session_config is None:
        gpu_options = tf.GPUOptions(allow_growth=True)
        session_config = tf.ConfigProto(
            allow_soft_placement=True,  # GPU에서 지원되지 않는 연산은 CPU로 자동 이동
    ...
    # 데이터셋 핸들 등록
    if hasattr(model, 'dataset_iterators') and model.datasets:
        model.dataset_handles = {}
        for n, i in model.dataset_iterators.items():
            model.dataset_handles[n] = model.sess.run(i.string_handle())
